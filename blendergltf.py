...
def export_meshes(settings, meshes, mesh_props, skinned_meshes):
    ...
    def export_fake_mesh(me, props):
        ...
        if vertex_access is None:
            settings['report']({'WARNING'},'Vertex accessor for fake mesh was'
                                ' assigned a generated name that may change in'
                                ' the future, set ' + REDC_VERTEX_ACCESS_NAME +
                                ' to give it a name.')
        if index_access is None:
            settings['report']({'WARNING'},'Index accessor for fake mesh was'
                               ' assigned a generated name that may change in'
                               ' the future, set ' + REDC_INDEX_ACCESS_NAME +
                               ' to give it a name.')
    ...
    for me in meshes:
        ...
        props = mesh_props[me]
        if settings['use_redcrane_extensions'] and \
           props.get(REDC_IS_FAKE, False):
...
def export_gltf(scene_delta, settings={}):
    ...
    if settings['use_redcrane_extensions']:
